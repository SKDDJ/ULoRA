...
pid_file="/tmp/task_pids_large_lr"
# clear the last
rm /tmp/task_pids_large_lr
...
run_task() {
    ...
    pid=$!
    echo "$pid $gpu_index" >> /tmp/task_pids_large_lr
}
    ...
    for DATASET in "${DATASETS[@]}"; do
        while true; do
            ...
            if [ $gpu_index -ne -1 ]; then
                ...
                if [[ -s /tmp/task_pids_large_lr ]]; then
                    while read -r pid gpu_index; do
                        if ! kill -0 $pid 2>/dev/null; then
                            gpu_tasks[$gpu_index]=$((gpu_tasks[$gpu_index] - 1))
                            sed -i "/^$pid $gpu_index$/d" /tmp/task_pids_large_lr
                        fi
                    done < /tmp/task_pids_large_lr
                fi
            fi
        done
    done
