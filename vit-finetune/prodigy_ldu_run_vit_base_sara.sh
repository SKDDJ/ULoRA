...
for DATASET in "${DATASETS[@]}"; do 
    ...
    CUDA_VISIBLE_DEVICES=$gpu python main.py fit --config $CONFIG --seed_everything $SEED --trainer.accelerator gpu --trainer.devices=[0] --trainer.precision 16-mixed \
      ...
      --model.lora_r $R --model.lora_alpha $ALPHA --model.init_method $INIT_METHOD --model.init_sara_weights $INIT_SARA_WEIGHTS > ./logs/61-${INIT_METHOD}/vit_base_${DATASET}.log 2>&1
