...
pid_file="/tmp/task_pids_base_rank"
# clear the last
rm /tmp/task_pids_base_rank
echo "Starting Evaluations..."
date

MODEL_NAME="vit-b16-224-in21k"
NAME="base"
INIT_SARA_WEIGHTS="lora"
INIT_METHOD="lora"
LR_SCHEDULER="linear" 
OPT="prodigy"
...
run_task() {
    ...
    HF_ENDPOINT=https://hf-mirror.com  CUDA_VISIBLE_DEVICES=$gpu_id python /root/shiym_proj/Sara/vit-finetune/main.py fit --config $CONFIG --seed_everything $SEED --trainer.accelerator gpu --trainer.devices=[0] --trainer.precision 16-mixed \
        --trainer.max_epochs 10 --trainer.logger.name AAAProdigy-LoRA-RANK-$R-$NAME-$INIT_METHOD-$dataset-LR-$LR-SCHEDULER-$LR_SCHEDULER_NAME --model.warmup_steps 30 --model.lr $LR --model.scheduler $LR_SCHEDULER --model.optimizer $OPT \
        --data.batch_size 128 --data.dataset $dataset --data.workers 4 --model.model_name $MODEL_NAME \
        --model.lora_r $R --model.lora_alpha $R --model.init_method $INIT_METHOD --model.init_sara_weights $INIT_SARA_WEIGHTS > /root/shiym_proj/Sara/vit-finetune/logs/622-lora/vit_base_${R}_${dataset}.log 2>&1 &
    ...
    pid=$!
    echo "$pid $gpu_index" >> /tmp/task_pids_base_rank
}
    ...
    for DATASET in "${DATASETS[@]}"; do
        while true; do
            ...
            if [ $gpu_index -ne -1 ]; then
                ...
                if [[ -s /tmp/task_pids_base_rank ]]; then
                    while read -r pid gpu_index; do
                        if ! kill -0 $pid 2>/dev/null; then
                            gpu_tasks[$gpu_index]=$((gpu_tasks[$gpu_index] - 1))
                            sed -i "/^$pid $gpu_index$/d" /tmp/task_pids_base_rank
                        fi
                    done < /tmp/task_pids_base_rank
                fi
            fi
        done
    done
