...
run_task() {
    ...
    CUDA_VISIBLE_DEVICES=$gpu_id python /root/shiym_proj/Sara/vit-finetune/main.py fit --config $CONFIG --seed_everything $SEED --trainer.accelerator gpu --trainer.devices=[0] --trainer.precision 16-mixed \
        ...
        --model.lora_r $R --model.lora_alpha $R --model.init_method $INIT_METHOD --model.init_sara_weights $INIT_SARA_WEIGHTS > /root/shiym_proj/Sara/vit-finetune/logs/62-${INIT_METHOD}/vit_base_${R}_${dataset}.log 2>&1 &
