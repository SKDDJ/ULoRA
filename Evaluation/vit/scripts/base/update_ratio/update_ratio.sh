...
pid_file="/tmp/task_pids_base_update_ratio"
# clear the last
rm /tmp/task_pids_base_update_ratio
...
run_task() {
    ...
    HF_ENDPOINT=https://hf-mirror.com  CUDA_VISIBLE_DEVICES=$gpu_id python /root/shiym_proj/Sara/vit-finetune/main.py fit --config $CONFIG --seed_everything $SEED --trainer.accelerator gpu --trainer.devices=[0] --trainer.precision 16-mixed \
        --trainer.max_epochs 10 --trainer.logger.name Blora-Ratio-$ratio-R-$R-$NAME-$INIT_METHOD-$dataset-LR-$LR-SCHEDULER-$LR_SCHEDULER_NAME --model.warmup_steps 30 --model.lr $LR --model.scheduler $LR_SCHEDULER --model.optimizer $OPT \
        --data.batch_size 128 --data.dataset $dataset --data.workers 4 --model.model_name $MODEL_NAME \
        --model.lora_r $R --model.update_ratio $ratio --model.lora_alpha $R --model.init_method $INIT_METHOD --model.init_sara_weights $INIT_SARA_WEIGHTS > /root/shiym_proj/Sara/vit-finetune/logs/62-${INIT_METHOD}/vit_base_${ratio}_${dataset}.log 2>&1 &
    ...
    pid=$!
    echo "$pid $gpu_index" >> /tmp/task_pids_base_update_ratio
}
    ...
    for DATASET in "${DATASETS[@]}"; do
        while true; do
            ...
            if [ $gpu_index -ne -1 ]; then
                ...
                if [[ -s /tmp/task_pids_base_update_ratio ]]; then
                    while read -r pid gpu_index; do
                        if ! kill -0 $pid 2>/dev/null; then
                            gpu_tasks[$gpu_index]=$((gpu_tasks[$gpu_index] - 1))
                            sed -i "/^$pid $gpu_index$/d" /tmp/task_pids_base_update_ratio
                        fi
                    done < /tmp/task_pids_base_update_ratio
                fi
            fi
        done
    done
