...
def main(args):
        ...
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(model, type(unwrap_model(unet))):
                    unet_lora_layers_to_save = convert_state_dict_to_diffusers(sara_state_dict.__get__(model, type(model)))
                elif isinstance(model, type(unwrap_model(text_encoder))):
                    text_encoder_lora_layers_to_save = convert_state_dict_to_diffusers(
                        sara_state_dict.__get__(model, type(model))
                    )
                else:
    ...
    if accelerator.is_main_process:
        ...
        unet_lora_state_dict = convert_state_dict_to_diffusers(sara_state_dict.__get__(unet, type(unet)))

        if args.train_text_encoder:
            text_encoder = unwrap_model(text_encoder)
            text_encoder_state_dict = convert_state_dict_to_diffusers(sara_state_dict.__get__(text_encoder, type(text_encoder)))
        else:
        ...
        lora_path=false
        if lora_path:
            with safe_open(self.lora_path, framework="pt", device=self.device) as f:  # Adjust 'framework' and 'device' as necessary
                for k in f.keys():
            ...
            with monit.section("Merge_SaRA"):
                ...
                add_sara_by_name(self.model, target_module_names=target_modules,sara_config=sara_config)
                ...
                for name, param in self.model.named_parameters():
                        if 'layers.0' in name:
                            if 'q_proj' in name:
                                ...
                                print(f"{name}: {param.size()} , {param}")
                _ = self.model.load_state_dict(state_dict_to_save, strict=False)
                
                merge_sara(self.model) 
