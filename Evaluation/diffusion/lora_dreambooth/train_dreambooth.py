...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--validation_prompt",
        type=str,
        default="masterpiece, best quality, (detailed face, perfect eyes, realistic eyes, perfect fingers), fantasy girl with long hair and autumn leaf clips, outdoors during sunset in a red velvet dress, (clear face), full body, amidst falling leaves.",
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(f"instance_image_path: {self.instance_images_path[index % self.num_instance_images]}")
...
def main(args):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        if accelerator.is_main_process:
                            ...
                            # state_dict_save_path = os.path.join(save_path, 'sara_state_dict.pth')
                            # torch.save(state_dict, state_dict_save_path)
                            # logger.info(f"Saved state to {state_dict_save_path}")
                            
                            # full state_dict 3.3G 
                            # state_dict_save_path = os.path.join(save_path, 'unet_state_dict.pth')
                ...
                if (
                    ...
                    for _ in range(args.num_validation_images):
                        images.extend(pipeline(args.validation_prompt, num_inference_steps=25, generator=generator,guidance_scale=5,negative_prompt=args.negative_prompt).images)
                        # image = pipeline(args.validation_prompt, num_inference_steps=25, generator=generator,guidance_scale=5,negative_prompt=[args.negative_prompt]*len(args.validation_prompt)).images[0]
                        # images.append(image)
    ...
    if accelerator.is_main_process:
        if args.use_lora:
            ...
            unwarpped_unet.state_dict = sara_state_dict.__get__(unwarpped_unet, type(unwarpped_unet))
