...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--wandb_proj",
        type=str,
        default="Dreambooth-lora-xl",
        help=('feel free to change the project name to your liking'),
    )
    parser.add_argument(
        "--entity",
        type=str,
...
def main(args):
    ...
    with monit.section("loading tokenizers..."):
        # Load the tokenizers
    ...
    if "EDM" in scheduler_type:
        ...
        noise_scheduler = EDMEulerScheduler.from_config(noise_scheduler.config, timestep_spacing="trailing")

        logger.info("Performing EDMEuler EDM-style training!")
    elif args.do_edm_style_training:
        # Ensure sampler uses "trailing" timesteps.
        noise_scheduler = EulerDiscreteScheduler.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="scheduler"
        )
        noise_scheduler = EulerDiscreteScheduler.from_config(noise_scheduler.config, timestep_spacing="trailing")

        logger.info("Performing Trailing EulerDiscrete EDM-style training!")
    else:
        noise_scheduler = DDPMScheduler.from_pretrained(args.pretrained_model_name_or_path, subfolder="scheduler")
        noise_scheduler = DDPMScheduler.from_config(noise_scheduler.config, timestep_spacing="trailing")
        logger.info("Performing DDPM training!")
        
    with monit.section("loading text_encoders..."):
    ...
    with monit.section("loading lightning module..."):
        repo = "ByteDance/SDXL-Lightning"
        ckpt = "sdxl_lightning_4step_unet.safetensors" # Use the correct ckpt for your step setting!
        # here we use 4 steps models
        unet.load_state_dict(load_file(hf_hub_download(repo, ckpt), device="cpu")) # here we load lightning sdxl-lightning
    ...
    if args.use_lora:
        ...
        lora_dropout=0.
        lora_alpha=320
        # init_method="fast_niter_8"
        init_method="fast_niter_4"
        lora_target_modules=["to_q","to_k","to_v","to_out"]
        print(f"Using sara, sara_r :{lora_r}")
        print(f"Using sara, sara_alpha :{lora_alpha}")
        sara_config = {
            nn.Linear: {
                "weight": partial(SaRAParametrization.from_linear, rank=lora_r, lora_dropout_p=lora_dropout, lora_alpha=lora_alpha, init_sara_weights=init_method),
            },
        }        
        ...
        # vector_Z启用梯度
        for param in get_sara_params(unet):
            param.requires_grad = True
        print_vector_parameters(unet)
        for name, param in unet.named_parameters():
            print(f"{name}: requires_grad={param.requires_grad}")
        # exit()
    ...
    # todo:  here to look competition prompts loading
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if not os.path.exists(save_path):
                            ...
                            logger.info(f"Created directory at {save_path}")
                        # 解包模型并保存 state_dict
                        unwrapped_unet = accelerator.unwrap_model(unet)
                        state_dict = sara_state_dict(unwrapped_unet)
                        # 保存 state_dict 为 safetensors 格式
                        state_dict_save_path = os.path.join(save_path, 'unet_state_dict.safetensors')
                        save_file(state_dict, state_dict_save_path)  # 使用 safetensors 保存
                        logger.info(f"Saved sara state_dict to {state_dict_save_path}")
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                pipeline_args = {"prompt": args.validation_prompt, "num_inference_steps": 4, "guidance_scale": 2, "negative_prompt": args.negative_prompt}
                with monit.section("Validation"):
    ...
    if accelerator.is_main_process:
        ...
        unet.state_dict = sara_state_dict.__get__(unet, type(unet))
        ...
        with monit.section("loading sara safety for Inference"):
            # Read the .safetensors file
            state_dict_to_save = {}
            with safe_open(os.path.join(args.output_dir, "unet"), framework="pt", device=pipeline.device) as f:  # Adjust 'framework' and 'device' as necessary
                for k in f.keys():
        ...
        with monit.section("Merge_SaRA"):
            add_sara_by_name(pipeline.unet, target_module_names=target_modules,sara_config=sara_config)
            _ = pipeline.unet.load_state_dict(state_dict_to_save, strict=False)
            merge_sara(pipeline.unet) 

        # run inference
        images = []
        if args.validation_prompt and args.num_validation_images > 0:
            pipeline_args = {"prompt": args.validation_prompt, "num_inference_steps": 4, "guidance_scale": 2, "negative_prompt": args.negative_prompt}
            with monit.section("Final Validation"):
                images = log_validation(
                    pipeline,
                    args,
                    accelerator,
