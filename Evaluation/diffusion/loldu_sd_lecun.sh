...
while true; do
    for ((gpu_id=0; gpu_id<8; gpu_id++)); do
        if [ $gpu_id -ne 4 ]; then
            ...
            precision="fp16"
            export WANDB_NAME=lecun-loldu-diffusion-$learning_rate
            output_dir="/root/shiym_proj/Sara/Evaluation/diffusion/lora_dreambooth/outputs/lecun/$precision-727"

            CUDA_VISIBLE_DEVICES=$gpu_id accelerate launch --mixed_precision=$precision train_dreambooth.py \
                --pretrained_model_name_or_path=$MODEL_NAME \
                --pretrained_vae_model_name_or_path=$VAE_NAME \
                --instance_data_dir="./datasets/yann" \
                ...
                --validation_prompt="a sks man wearing a helmet" \
