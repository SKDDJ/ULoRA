...
class SaRAParametrization(pl.LightningModule):
    def __init__(self, fan_in, fan_out, fan_in_fan_out=False, rank=4, lora_dropout_p=0.0, lora_alpha=8, layer_module=None, init_sara_weights="fast_niter_8", init_method="svd", update_ratio=0.5 ):
        super().__init__() 
    ...
    def get_residual_matrix(self):
        ...
        if init_sara_weights == "svd":
            ...
            Sr /= self.scaling
            Uhr = Uh[: r]
            print("I'm here because I'm using svd successfully")
            ...
            Sr /= self.scaling
            Uhr = Ur.t()
            print("I'm here because I'm using fast-svd successfully")
    ...
    @classmethod
    def from_linear(cls, layer, rank=4, lora_dropout_p=0.0, lora_alpha=1, init_sara_weights="fast_niter_8", init_method="svd",update_ratio=0.5):
        fan_out, fan_in = layer.weight.shape        
        return cls(
            fan_in, fan_out, fan_in_fan_out=False, rank=rank, lora_dropout_p=lora_dropout_p, lora_alpha=lora_alpha, layer_module=layer, init_sara_weights="fast_niter_8", init_method=init_method, update_ratio=update_ratio 
        )
